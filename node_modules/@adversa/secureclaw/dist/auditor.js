...
import { loadIOCDatabase, isKnownC2, isKnownMaliciousDomain, isKnownMaliciousHash, matchesTyposquat, matchesDangerousPattern, getInfostealerArtifacts, } from './utils/ioc-db.js';
// ============================================================
 ...
 */
export function probePort(port, host, timeoutMs = 2000) {
...
// ============================================================
export async function auditGateway(ctx, deep = false) {
    const findings = [];
...
// ============================================================
export async function auditCredentials(ctx) {
    const findings = [];
    ...
    for (const match of allFiles) {
        ...
        if (match.includes('soul.md') || match.includes('MEMORY.md') || match.includes('SOUL.md'))
            continue;
...
// ============================================================
export async function auditExecution(ctx) {
    const findings = [];
...
// ============================================================
export async function auditAccessControl(ctx) {
    const findings = [];
...
// ============================================================
export async function auditSupplyChain(ctx) {
    const findings = [];
...
// ============================================================
export async function auditMemoryIntegrity(ctx) {
    const findings = [];
    const agentsDir = path.join(ctx.stateDir, 'agents');
    let agents = [];
    try {
        ...
        for (const line of lines) {
            try {
                ...
                if (entry.inputTokens)
                    totalTokens += entry.inputTokens;
                if (entry.outputTokens)
                    totalTokens += entry.outputTokens;
                if (entry.estimatedCostUsd)
...
// ============================================================
export async function auditIOC(ctx) {
    const findings = [];
    let db;
    try {
        db = await loadIOCDatabase();
...
// ============================================================
export async function auditMultiFramework(ctx) {
    const findings = [];
...
export async function runAudit(options = {}) {
    ...
    const [gatewayFindings, credentialFindings, executionFindings, accessControlFindings, supplyChainFindings, memoryFindings, costFindings, iocFindings, multiFrameworkFindings,] = await Promise.all([
