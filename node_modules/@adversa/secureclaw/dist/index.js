 ...
 */
export async function isKillSwitchActive(stateDir) {
 ...
 */
export async function activateKillSwitch(stateDir, reason) {
 ...
 */
export async function deactivateKillSwitch(stateDir) {
    const killPath = path.join(stateDir, '.secureclaw', 'killswitch');
    try {
 ...
 */
export async function logToolCall(stateDir, toolName, dataPath) {
 ...
 */
export async function getBehavioralBaseline(stateDir, windowMinutes = 60) {
        ...
        api.on('gateway_start', async () => {
            try {
                ...
                if (report.summary.critical > 0) {
                    api.logger.warn(`[SecureClaw] WARNING: ${report.summary.critical} CRITICAL finding(s) detected!`);
                }
                ...
                api.logger.error(`[SecureClaw] Startup audit failed: ${err.message}`);
                api.logger.error(`[SecureClaw] Stack: ${err.stack}`);
...
// ============================================================
async function onGatewayStart(gateway) {
...
export const legacyPlugin = {
    ...
    commands: {
        ...
        'secureclaw scan-skill': async (...args) => {
        ...
        'secureclaw baseline': async (...args) => {
            const stateDir = process.env['OPENCLAW_STATE_DIR'] ?? path.join(os.homedir(), '.openclaw');
        ...
        'secureclaw skill uninstall': async () => {
            ...
            try {
                ...
                console.error('Failed to uninstall skill:', err.message);
            }
        },
    },
