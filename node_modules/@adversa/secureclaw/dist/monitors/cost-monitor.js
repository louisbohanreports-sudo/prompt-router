 ...
 */
export function parseSessionLog(content) {
    const entries = [];
 ...
 */
export function calculateCostForWindow(entries, windowMs) {
    const now = Date.now();
 ...
 */
export function generateCostReport(entries) {
...
export function checkLimits(entries) {
    ...
    if (entries.length > 0) {
        ...
        const recentEntries = entries.filter((e) => Date.now() - new Date(e.timestamp).getTime() < 60 * 60 * 1000);
 ...
 */
export function addCostEntries(entries) {
    costEntries.push(...entries);
}
/**
 ...
 */
export function resetCostMonitor() {
    alerts = [];
    alertCallbacks = [];
    running = false;
    lastCheck = undefined;
