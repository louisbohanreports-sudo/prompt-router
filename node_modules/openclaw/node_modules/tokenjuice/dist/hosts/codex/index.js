...
const FEATURE_FLAG_NAMES = ["hooks", "codex_hooks"];
const FEATURE_FLAG_FIX_HINT = "Codex hooks are disabled. Enable per-invocation with `codex exec --enable hooks ...`, " +
    "or persistently by adding a `[features]` section with `hooks = true` to ~/.codex/config.toml.";
function buildCodexFeatureFlagStatus(configPath, configExists) {
    return {
        configPath,
        configExists,
        keyPresent: false,
 ...
 */
export async function inspectCodexHooksFeatureFlag(configPath = getDefaultCodexConfigPath()) {
    let source;
    try {
 ...
 */
export function parseCodexFeatureFlag(source, keys) {
    ...
    for (const rawLine of lines) {
        ...
        if (dotted) {
            return { keyPresent: true, key: dotted[1], value: dotted[2] === "true" };
        }
        if (currentTablePath.length === 1 && currentTablePath[0] === "features") {
            const scoped = scopedRe.exec(line);
            if (scoped) {
                return { keyPresent: true, key: scoped[1], value: scoped[2] === "true" };
            }
        }
    }
    return { keyPresent: false, value: null };
}
export async function inspectCodexRuntimeConfig(configPath = getDefaultCodexConfigPath()) {
    let source;
    try {
        source = await readFile(configPath, "utf8");
    }
...
function parseCodexRuntimeConfig(source) {
    const values = {
        approvalPolicy: null,
        sandboxMode: null,
        approvalsReviewer: null,
...
async function buildCodexHookCommand(options = {}) {
...
async function detectStaleLocalBuild(commandPaths) {
    ...
    const latestSourceMtimeMs = Math.max(await newestMtimeMs(join(projectRoot, "src")) ?? 0, await newestMtimeMs(join(projectRoot, "package.json")) ?? 0, await newestMtimeMs(join(projectRoot, "tsconfig.json")) ?? 0);
...
function createTokenjuiceCodexHook(command) {
    return {
        matcher: "^Bash$",
        hooks: [
            {
...
function findTokenjuiceCodexHook(config) {
...
function truncateHookCommand(command, maxLength = 80) {
...
function extractHomebrewCellarVersion(path) {
    const normalized = path.replace(/\\/gu, "/");
    const match = normalized.match(/\/Cellar\/tokenjuice\/([^/]+)\//u);
    return match?.[1];
}
async function detectResolvedHomebrewVersionMismatch(commandPaths) {
    ...
    for (const [eventName, groups] of Object.entries(config.hooks)) {
        groups.forEach((group, groupIndex) => {
            ...
            group.hooks.forEach((hook, hookIndex) => {
                ...
                issues.push(`non-tokenjuice Codex hook ${location} uses a low ${hook.timeout}s timeout for "${command}" — consider ${RECOMMENDED_NON_TOKENJUICE_TIMEOUT_SECONDS}s or higher`);
            });
        });
    }
    return issues;
...
function sanitizeHooksConfig(raw) {
    ...
    for (const [eventName, groups] of Object.entries(raw.hooks)) {
        ...
        const normalizedGroups = groups.flatMap((group) => {
            ...
            const commands = group.hooks.flatMap((hook) => {
                if (!isRecord(hook)) {
                    return [];
                }
                return [{ ...hook }];
...
async function loadHooksConfig(hooksPath) {
    try {
...
function isTokenjuiceExecutableArg(value) {
    if (!value) {
        return false;
    }
...
function buildCodexFeedback(inlineText, rawRefId) {
    return `${inlineText}\n\n${buildCompactionHint(rawRefId)}`;
}
function buildCodexReplacementOutput(inlineText, rawRefId) {
...
function parseExitCodeValue(value) {
...
export async function doctorCodexHook(hooksPath = getDefaultHooksPath(), options = {}) {
    ...
    if (resolvedVersionMismatch) {
        issues.push(`configured Codex hook launcher resolves to Homebrew tokenjuice ${resolvedVersionMismatch.resolvedVersion}, but this tokenjuice is ${packageJson.version}`);
    ...
    if (!featureFlag.enabled) {
        issues.push("Codex feature flag `hooks` is disabled — the configured hook will not fire");
    }
...
async function writeHookDebug(record) {
...
function sanitizeHookHistoryLines(text) {
    return text
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean)
...
async function acquireHistoryLock(lockPath) {
...
async function writeHookHistoryEntry(historyPath, line) {
...
function buildImmediateSkipStats(text) {
...
async function recordImmediateHookStats(input, rawText, storeRaw) {
    if (storeRaw) {
        return;
    }
