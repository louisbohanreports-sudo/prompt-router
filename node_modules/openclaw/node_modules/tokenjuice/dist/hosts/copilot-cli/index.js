...
import { ensureHooksDir, findStrayTokenjuiceHookFiles, isRecord, loadCopilotHooksConfigWithBackup, readCopilotHooksConfig, writeCopilotHooksConfigAtomic, } from "../shared/hooks-json-file.js";
...
const TOKENJUICE_COPILOT_CLI_INSTRUCTIONS_ADVISORY = "paste the snippet from `tokenjuice doctor copilot-cli --print-instructions` into `.github/copilot-instructions.md` or `AGENTS.md` so the agent respects compacted output and knows when to prefix a command with `tokenjuice wrap --raw --`.";
...
`;
export function getCopilotCliInstructionsSnippet() {
...
function getCopilotCliHooksDir() {
...
async function buildCopilotCliHookCommand(options = {}) {
...
export async function installCopilotCliHook(hooksPath = getDefaultHooksPath(), options = {}) {
...
export async function uninstallCopilotCliHook(hooksPath = getDefaultHooksPath()) {
...
async function findStrayCopilotCliHookFiles(hooksDir, canonicalPath) {
    return findStrayTokenjuiceHookFiles(hooksDir, canonicalPath, (command) => command.includes(TOKENJUICE_COPILOT_CLI_SUBCOMMAND));
}
export async function doctorCopilotCliHook(hooksPath = getDefaultHooksPath(), options = {}) {
    ...
    if (missingPaths.length > 0) {
        issues.push(`configured copilot-cli hook points at missing path${missingPaths.length === 1 ? "" : "s"}`);
    }
...
export async function runCopilotCliPostToolUseHook(rawText) {
    let payload;
    try {
