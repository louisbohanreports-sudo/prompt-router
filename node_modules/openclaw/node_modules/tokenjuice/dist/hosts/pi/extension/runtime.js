...
function buildLargeDocumentSummary(lines, rawText) {
  ...
  const clippedHeadings = headings.map((line2) => {
    const clipped = clipMiddleWithHash(line2, 180);
...
function rewriteSearchLines(lines) {
  ...
  const rewritten = lines.map((line2) => {
    ...
    if (!match) {
      const clipped2 = clipMiddleWithHash(line2, LONG_SEARCH_LINE_MAX_CHARS);
...
function rewriteGitDiffLines(lines) {
  ...
  for (const line2 of lines) {
    ...
    if (emittedChangedLinesInHunk < GIT_DIFF_CHANGED_LINES_PER_HUNK) {
      ...
      const clipped = clipMiddleWithHash(line2, LONG_CHANGED_LINE_MAX_CHARS);
      if (clipped.compaction) {
        compactions.push(clipped.compaction);
      }
...
function rewriteGhLines(lines, input) {
  ...
  if (parsedJsonLines.every((entry) => entry !== null)) {
    const compactions = [];
    const rewritten = parsedJsonLines.map((entry) => formatGhJsonRecord(entry)).flatMap((line2) => {
